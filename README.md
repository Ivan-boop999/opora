...
Повторный запуск seed безопасен. Команда запрещает `NODE_ENV=production` и принимает только локальный loopback-адрес PostgreSQL. В ветке `mobile` она также выдаёт обычному демопользователю локальный premium-доступ, чтобы сразу открыть основной экран компонентов. Админского интерфейса в mobile нет.
