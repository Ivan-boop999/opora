...
Прод-деплой — Render free: API собирается из `backend/Dockerfile` (миграции Prisma при старте), webapp публикуется как статический сайт, БД — Neon. Выбор зафиксирован в [CHECKLIST.md](../CHECKLIST.md). Локальной разработке облачные ключи не нужны.

Инфраструктура Terraform удалена при установке: деплой на Render free (записи в CHECKLIST.md и README.md).
...
До плана основы скрипт сравнивает версию и отпечаток пароля активного слота с прежним state. Заменять его запрещено. Обнови и выбери неактивный слот, примени основу, затем выполни релиз. При неудаче старый runtime сохранит логин и секрет. Команды — в YANDEX_CLOUD.md.
