...
const imageName = process.env.BACKEND_DOCKER_SMOKE_IMAGE ?? 'opora-backend:smoke'
const containerName =
  process.env.BACKEND_DOCKER_SMOKE_CONTAINER ??
  `opora-backend-smoke-${repositoryHash}-${process.pid}`
...
const databaseUrlForContainer =
  ...
  'postgresql://superuser:superpassword@postgres_test:5432/opora_test?schema=public'
