...
React, TypeScript, Vite, Tailwind CSS, shadcn/ui, Radix UI, TanStack Query/Form/Router, Zod из `@opora/contracts`, shadcn CLI, Playwright и ESLint.
...
Скрипт запускает `postgres_test`, применяет миграции к `opora_test`, создаёт E2E-администратора и запускает backend с `DATABASE_URL` из `TEST_DATABASE_URL`. Затем запускает Vite. После прогона он по умолчанию удаляет том тестовой БД.
